...
namespace EClient.Utilities
{
    // Token: 0x02000008 RID: 8
    internal class Rig : MonoBehaviour
    {
        ...
        public static VRRig GetRigFromPlayer(Player p)
        {
            foreach (NetPlayer netPlayer in NetworkSystem.Instance.AllNetPlayers)
            {
                ...
                if (flag)
                {
                    return Rig.GetRigFromNetPlayer(netPlayer);
                }
            }
            return null;
        }
        ...
        {
            return Rig.rig2view(GorillaGameManager.instance.FindPlayerVRRig(p));
        }
        ...
        {
            Vector2 vector = new(a.x, a.z);
            Vector2 vector21 = new (a.x, a.z);
            Vector2 vector2 = new(a.x, a.z);
            Vector2 vector22 = new (b.x, b.z);
        ...
        {
            this.layerMask = (UnityLayerExtensions.ToLayerMask(0) | UnityLayerExtensions.ToLayerMask(0));
        ...
        public static Player GetRandomPlayer(bool includeSelf)
        {
            ...
            if (includeSelf)
            {
                Player player = PhotonNetwork.PlayerList[UnityEngine.Random.Range(0, 11)];
                ...
                {
                    result = Rig.GetRandomPlayer(includeSelf);
                }
            }
            else
            {
                Player player2 = PhotonNetwork.PlayerListOthers[UnityEngine.Random.Range(0, 10)];
        ...
        public static NetPlayer GetRandomNetPlayer(bool includeSelf)
        {
            ...
            if (includeSelf)
            {
                NetPlayer netPlayer = NetworkSystem.Instance.PlayerListOthers[UnityEngine.Random.Range(0, NetworkSystem.Instance.PlayerListOthers.Length)];
                ...
                {
                    result = Rig.GetRandomNetPlayer(includeSelf);
                }
            }
            else
            {
                NetPlayer netPlayer2 = NetworkSystem.Instance.PlayerListOthers[UnityEngine.Random.Range(0, NetworkSystem.Instance.PlayerListOthers.Length)];
